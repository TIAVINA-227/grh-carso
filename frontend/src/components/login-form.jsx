...
// ========================================
// 2. frontend/src/components/login-form.jsx (CORRIGÉ)
// ========================================
import React, { useState } from "react"
import { cn } from "@/lib/utils"
import { Button } from "@/components/ui/button"
...
export function LoginForm({ className, ...props }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log("🔐 Première connexion?", data.user?.premiere_connexion || data.user?.premiereConnexion)
      ...
      login(data.token, {
        ...
        nom_utilisateur: data.user?.nom_utilisateur,
        prenom_utilisateur: data.user?.prenom_utilisateur,
        premiere_connexion: data.user?.premiere_connexion,     // ✅ snake_case
        premiereConnexion: data.user?.premiereConnexion        // ✅ camelCase
      })
      ...
      // Redirection (la gestion de la première connexion se fera dans App.jsx)
      navigate("/dashboard", { replace: true })
